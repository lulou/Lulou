...
-   **Elevate System**: Stores boost state in the local `user_elevates` table (not Supabase, since Supabase `profiles` lacks those columns).
    ...
    -   Elevate = 30-minute boost ($9.99); Super Elevate = 60-minute boost ($34.99).
    -   **Stripe payment flow**: `POST /api/stripe/elevate-checkout` creates a Stripe Checkout session and returns a hosted payment URL. On success, Stripe redirects to `/elevate/success?session_id=XXX`. The `ElevateSuccessPage` polls `POST /api/stripe/elevate-activate` to verify payment and then calls `activateElevate` to apply the boost. A webhook at `POST /api/stripe/webhook` processes Stripe events via `stripe-replit-sync`.
    -   **Two-step purchase flow** in `ElevateModal` (`client/src/components/elevate-modal.tsx`): Browse step (pricing) → Checkout step (order summary + "Pay [price]" button). Pressing Pay redirects the user to Stripe-hosted checkout.
