...
import { welcomeEmail, emailVerificationLinkEmail } from "./emailTemplates";
...
// ── Application-level signup / resend rate limiting ───────────────────────────
// These guards run BEFORE any Supabase call so our limits protect the provider
// email budget.  All thresholds are configurable via env vars so ops can tune
// without a redeploy.  In-memory only — resets on server restart, which is
// intentional: a restart clears burst windows without permanently blocking.
//
// Signup limits (per email address):
//   • SIGNUP_COOLDOWN_MS   — minimum ms between two signup attempts for the
//                            same address (default 60 s).
//   • SIGNUP_MAX_EMAIL_HR  — max signup attempts per address per hour (default 3).
// Signup limits (per IP / X-Forwarded-For first hop):
//   • SIGNUP_MAX_IP_HR     — max signup attempts per IP per hour (default 10).
// Resend limits:
//   • RESEND_COOLDOWN_MS   — minimum ms between two resends for the same address
//                            (default 60 s).
//   • RESEND_MAX_EMAIL_DAY — max resends per address per 24 h (default 5).
//   • RESEND_MAX_IP_DAY    — max resends per IP per 24 h (default 20).
const _SU_COOLDOWN_MS  = parseInt(process.env.SIGNUP_COOLDOWN_MS   ?? "60000", 10);
const _SU_MAX_EMAIL_HR = parseInt(process.env.SIGNUP_MAX_EMAIL_HR  ?? "3",     10);
const _SU_MAX_IP_HR    = parseInt(process.env.SIGNUP_MAX_IP_HR     ?? "10",    10);
const _RE_COOLDOWN_MS  = parseInt(process.env.RESEND_COOLDOWN_MS   ?? "60000", 10);
const _RE_MAX_EMAIL_DY = parseInt(process.env.RESEND_MAX_EMAIL_DAY ?? "5",     10);
const _RE_MAX_IP_DY    = parseInt(process.env.RESEND_MAX_IP_DAY    ?? "20",    10);

interface _RLBucket { count: number; windowStart: number; lastAt: number }
const _suEmailBuckets = new Map<string, _RLBucket>();
const _suIpBuckets    = new Map<string, _RLBucket>();
const _reEmailBuckets = new Map<string, _RLBucket>();
const _reIpBuckets    = new Map<string, _RLBucket>();

// Non-reversible hash for diagnostic logs — never log a full email address.
function _diagHash(s: string): string {
  let h = 5381;
...
type _RLResult =
  | { ok: true }
  | { ok: false; reason: "email-cooldown" | "email-limit" | "ip-limit"; waitMs: number };

function _checkSignupRL(email: string, ip: string): _RLResult {
  const now = Date.now(); const HR = 3_600_000;
  const eb = _suEmailBuckets.get(email);
  if (eb) {
    if (now - eb.lastAt < _SU_COOLDOWN_MS)
      return { ok: false, reason: "email-cooldown", waitMs: _SU_COOLDOWN_MS - (now - eb.lastAt) };
    if (now - eb.windowStart < HR && eb.count >= _SU_MAX_EMAIL_HR)
      return { ok: false, reason: "email-limit", waitMs: HR - (now - eb.windowStart) };
  }
  const ib = _suIpBuckets.get(ip);
  if (ib && now - ib.windowStart < HR && ib.count >= _SU_MAX_IP_HR)
    return { ok: false, reason: "ip-limit", waitMs: HR - (now - ib.windowStart) };
  return { ok: true };
}

function _recordSignup(email: string, ip: string) {
  const now = Date.now(); const HR = 3_600_000;
  const eb = _suEmailBuckets.get(email);
  if (!eb || now - eb.windowStart >= HR) _suEmailBuckets.set(email, { count: 1, windowStart: now, lastAt: now });
  else { eb.count++; eb.lastAt = now; }
  const ib = _suIpBuckets.get(ip);
  if (!ib || now - ib.windowStart >= HR) _suIpBuckets.set(ip, { count: 1, windowStart: now, lastAt: now });
  else { ib.count++; ib.lastAt = now; }
}

function _checkResendRL(email: string, ip: string): _RLResult {
  const now = Date.now(); const DY = 86_400_000;
  const eb = _reEmailBuckets.get(email);
  if (eb) {
    if (now - eb.lastAt < _RE_COOLDOWN_MS)
      return { ok: false, reason: "email-cooldown", waitMs: _RE_COOLDOWN_MS - (now - eb.lastAt) };
    if (now - eb.windowStart < DY && eb.count >= _RE_MAX_EMAIL_DY)
      return { ok: false, reason: "email-limit", waitMs: DY - (now - eb.windowStart) };
  }
  const ib = _reIpBuckets.get(ip);
  if (ib && now - ib.windowStart < DY && ib.count >= _RE_MAX_IP_DY)
    return { ok: false, reason: "ip-limit", waitMs: DY - (now - ib.windowStart) };
  return { ok: true };
}

function _recordResend(email: string, ip: string) {
  const now = Date.now(); const DY = 86_400_000;
  const eb = _reEmailBuckets.get(email);
  if (!eb || now - eb.windowStart >= DY) _reEmailBuckets.set(email, { count: 1, windowStart: now, lastAt: now });
  else { eb.count++; eb.lastAt = now; }
  const ib = _reIpBuckets.get(ip);
  if (!ib || now - ib.windowStart >= DY) _reIpBuckets.set(ip, { count: 1, windowStart: now, lastAt: now });
  else { ib.count++; ib.lastAt = now; }
}
...
export async function registerRoutes(
  ...
  // ── Managed signup endpoint ───────────────────────────────────────────────
  // Routes signup through our server so we can:
  //   1. Apply application-level rate limits BEFORE touching Supabase.
  //   2. Create the auth user via the admin API so Supabase sends NO email.
  //   3. Generate a verification link via admin.generateLink and send it
  //      through Resend — completely bypassing Supabase's low default quota.
  //   4. Return categorised status codes the client can display specifically.
  //
  // Falls back gracefully when service-role key or RESEND_API_KEY is absent.
  // The client detects "use-direct-signup" and falls through to the direct
  // supabase.auth.signUp() call instead, preserving the existing behaviour.
  app.post("/api/auth/signup", async (req: any, res) => {
    const reqId = randomUUID().slice(0, 8);
    try {
      const { email, password, redirectTo } = req.body ?? {};
      if (!email || !password)
        return res.status(400).json({ status: "invalid-input", message: "Email and password are required." });
      ...
      console.log(`[AUTH_EMAIL_DIAG] action=signup reqId=${reqId} emailHash=${_diagHash(emailNorm)} ip=${ip.slice(0, 8)}…`);

      // ── Application-level rate limiting ─────────────────────────────────
      const rl = _checkSignupRL(emailNorm, ip);
      if (!rl.ok) {
        const waitSec = Math.ceil(rl.waitMs / 1000);
        logEmailEvent({ type: "signup_otp_rate_limited", email: emailNorm.slice(0, 4) + "***", success: false, note: `reqId=${reqId} reason=${rl.reason}` });
        console.log(`[AUTH_EMAIL_DIAG] signup_rate_limited reqId=${reqId} reason=${rl.reason} waitSec=${waitSec}`);
        const msgs: Record<string, string> = {
          "email-cooldown": `We recently sent a verification email to this address. You can request another in ${waitSec} seconds.`,
          "email-limit":    `Too many signup attempts for this address. Please try again in ${Math.ceil(rl.waitMs / 60000)} minutes.`,
          "ip-limit":       "Too many signup requests from your network. Please wait a few minutes and try again.",
        };
        return res.status(429).json({ status: `rate-limited-${rl.reason}`, message: msgs[rl.reason] ?? "Too many requests.", waitMs: rl.waitMs });
      }

      // Record the attempt BEFORE calling Supabase so a rapid second request
      // from the same email is blocked even if the first is still in-flight.
      _recordSignup(emailNorm, ip);

      // ── Require service-role key for admin API calls ─────────────────────
      if (!hasServiceRoleKey) {
        console.warn(`[AUTH_EMAIL_DIAG] no_service_role_key reqId=${reqId} — falling back to direct signup`);
        return res.json({ status: "use-direct-signup", message: "Server-side signup not available (no service role key)." });
      }

      // ── Create user via admin API (Supabase sends no email) ──────────────
      let userId: string | undefined;
      let isNewUser = true;

      {
        const { data: cuData, error: cuErr } = await supabaseAdmin.auth.admin.createUser({
          email: emailNorm,
        ...
        if (cuErr) {
          ...
          const alreadyReg = msg.includes("already registered") || msg.includes("already been registered") || cuErr.status === 422;
          if (!alreadyReg) {
            console.error(`[AUTH_EMAIL_DIAG] createUser_failed reqId=${reqId}`, cuErr.message);
            return res.status(500).json({ status: "error", message: "Failed to create account. Please try again shortly." });
          }
          // User exists — fall through to generateLink
          isNewUser = false;
        } else {
          userId = cuData.user?.id;
        }
      }

      // ── Generate verification link ────────────────────────────────────────
      const appRedirectTo =
        String(redirectTo || "").startsWith("http")
          ? String(redirectTo)
          : `${req.headers.origin ?? "https://lulou.app"}/auth/callback`;

      let verifyUrl: string | undefined;
      let isAlreadyVerified = false;

      try {
        const { data: lnkData, error: lnkErr } = await (supabaseAdmin.auth.admin as any).generateLink({
          type: "signup",
          email: emailNorm,
          options: { redirectTo: appRedirectTo },
        });
        if (lnkErr) {
          const lnkMsg = (lnkErr.message ?? "").toLowerCase();
          if (lnkMsg.includes("confirmed") || lnkMsg.includes("already verified") || lnkMsg.includes("already registered")) {
            isAlreadyVerified = true;
          } else {
            throw lnkErr;
          }
        } else {
          verifyUrl = lnkData?.properties?.action_link ?? lnkData?.action_link;
          if (!verifyUrl) throw new Error("generateLink returned no action_link");
          if (!userId) userId = lnkData?.user?.id;
        }
      } catch (err: any) {
        console.error(`[AUTH_EMAIL_DIAG] generateLink_failed reqId=${reqId}`, err.message);
        // Cannot generate the link — tell client to fall back
        return res.json({ status: "use-direct-signup", message: "Could not generate verification link." });
      }
      ...
      // ── Send via Resend ───────────────────────────────────────────────────
      const firstName = emailNorm.split("@")[0];
      const sent = await sendEmail({
        to: emailNorm,
        subject: "Verify your Lulou account",
        html: emailVerificationLinkEmail(firstName, verifyUrl!),
        type: "signup_verification",
      });

      if (!sent) {
        // Resend not configured or unavailable — tell client to use direct signUp
        console.warn(`[AUTH_EMAIL_DIAG] resend_unavailable reqId=${reqId} — falling back to direct signup`);
        return res.json({ status: "use-direct-signup", message: "Email service unavailable — using direct signup." });
      }

      logEmailEvent({ type: "signup_otp_sent", email: emailNorm.slice(0, 4) + "***", userId: userId?.slice(0, 8), success: true, note: `reqId=${reqId} isNewUser=${isNewUser} via=resend` });
      console.log(`[AUTH_EMAIL_DIAG] signup_complete reqId=${reqId} emailHash=${_diagHash(emailNorm)} isNewUser=${isNewUser} via=resend`);
      return res.json({ status: "verification-sent", isNewUser });

    } catch (err: any) {
      console.error(`[AUTH_EMAIL_DIAG] signup_unexpected_error reqId=${reqId}`, err?.message);
      return res.status(500).json({ status: "error", message: "An unexpected error occurred. Please try again." });
    }
  });

  // ── Managed resend-verification endpoint ─────────────────────────────────
  // Generates a fresh Supabase verification link via the admin API and
  // delivers it via Resend.  Enforces per-email + per-IP rate limits.
  // Falls back to "use-direct-resend" so the client can call supabase.auth.resend()
  // directly when the admin API or Resend is unavailable.
  app.post("/api/auth/resend-verification", async (req: any, res) => {
    const reqId = randomUUID().slice(0, 8);
    try {
      const { email, redirectTo } = req.body ?? {};
      if (!email)
        return res.status(400).json({ status: "invalid-input", message: "Email is required." });
      ...
      console.log(`[AUTH_EMAIL_DIAG] action=resend reqId=${reqId} emailHash=${_diagHash(emailNorm)} ip=${ip.slice(0, 8)}…`);

      // ── Rate limiting ─────────────────────────────────────────────────────
      const rl = _checkResendRL(emailNorm, ip);
      if (!rl.ok) {
        const waitSec = Math.ceil(rl.waitMs / 1000);
        logEmailEvent({ type: "resend_rate_limited", email: emailNorm.slice(0, 4) + "***", success: false, note: `reqId=${reqId} reason=${rl.reason}` });
        console.log(`[AUTH_EMAIL_DIAG] resend_rate_limited reqId=${reqId} reason=${rl.reason} waitSec=${waitSec}`);
        const msgs: Record<string, string> = {
          "email-cooldown": `We recently sent a verification email. You can request another in ${waitSec} seconds.`,
          "email-limit":    "You've requested too many verification emails today. Please check your inbox and spam folder.",
          "ip-limit":       "Too many requests from your network. Please try again later.",
        };
        return res.status(429).json({ status: `rate-limited-${rl.reason}`, message: msgs[rl.reason] ?? "Too many requests.", waitMs: rl.waitMs });
      }

      _recordResend(emailNorm, ip);

      if (!hasServiceRoleKey) {
        return res.json({ status: "use-direct-resend" });
      }
      ...
      try {
        ...
        if (lnkErr) {
          const m = (lnkErr.message ?? "").toLowerCase();
          if (m.includes("confirmed") || m.includes("already verified") || m.includes("already registered")) {
            return res.status(409).json({ status: "already-verified", message: "This email is already verified. Please sign in." });
          }
          throw lnkErr;
        }
        verifyUrl = lnkData?.properties?.action_link ?? lnkData?.action_link;
        ...
        console.error(`[AUTH_EMAIL_DIAG] resend_generateLink_failed reqId=${reqId}`, err.message);
        return res.json({ status: "use-direct-resend" });
      }
      ...
      if (!sent) {
        logEmailEvent({ type: "resend_failed", email: emailNorm.slice(0, 4) + "***", success: false, note: `reqId=${reqId} resend_key_missing` });
        return res.json({ status: "use-direct-resend" });
      }

      logEmailEvent({ type: "resend_queued", email: emailNorm.slice(0, 4) + "***", success: true, note: `reqId=${reqId} via=resend` });
      console.log(`[AUTH_EMAIL_DIAG] resend_complete reqId=${reqId} emailHash=${_diagHash(emailNorm)} via=resend`);
      return res.json({ status: "verification-sent" });

    } catch (err: any) {
      console.error(`[AUTH_EMAIL_DIAG] resend_unexpected_error reqId=${reqId}`, err?.message);
      return res.status(500).json({ status: "error", message: "An unexpected error occurred. Please try again." });
    }
  });
