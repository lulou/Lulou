...
export async function registerRoutes(
  ...
  app.post("/api/internal/run-migration", async (req: any, res) => {
    ...
    // Use Supabase's pg_meta HTTP API which is hosted per-project and accepts
    // the service role key to run arbitrary SQL.
    ...
    // Build the full migration SQL matching shared/schema.ts exactly.
    const migrationStatements = [
      ...
      `CREATE TABLE IF NOT EXISTS public.profiles (
        ...
        dating_preference     text        NOT NULL DEFAULT '',
        location              text        NOT NULL DEFAULT '',
        height                text,
        photos                text[]      NOT NULL DEFAULT '{}',
        signals               text[]      NOT NULL DEFAULT '{}',
        dating_intent         text        NOT NULL DEFAULT '',
      ...
      END $$`,
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='profiles' AND policyname='profiles_read_completed') THEN
          CREATE POLICY profiles_read_completed ON public.profiles FOR SELECT
      ...
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='interactions' AND policyname='interactions_participants') THEN
          CREATE POLICY interactions_participants ON public.interactions FOR ALL
      ...
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='matches' AND policyname='matches_participants') THEN
          CREATE POLICY matches_participants ON public.matches FOR ALL
      ...
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='spin_standouts' AND policyname='spin_standouts_own') THEN
          CREATE POLICY spin_standouts_own ON public.spin_standouts FOR ALL
            USING (auth.uid()::text = user_id) WITH CHECK (auth.uid()::text = user_id);
        END IF;
      END $$`,
      // spin_usage
      ...
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='spin_usage' AND policyname='spin_usage_own') THEN
          CREATE POLICY spin_usage_own ON public.spin_usage FOR ALL
            USING (auth.uid()::text = user_id) WITH CHECK (auth.uid()::text = user_id);
        END IF;
      END $$`,
      // spin_requests
      ...
      `DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='spin_requests' AND policyname='spin_requests_participants') THEN
          CREATE POLICY spin_requests_participants ON public.spin_requests FOR ALL
            USING (auth.uid()::text = from_user_id OR auth.uid()::text = to_user_id)
            WITH CHECK (auth.uid()::text = from_user_id);
        END IF;
      END $$`,
    ...
    for (const sql of migrationStatements) {
      ...
      results.push({ sql: sql.substring(0, 60) + "...", status: r.status, body: r.body.substring(0, 200) });
      if (r.status >= 400 && !r.body.includes("already exists")) {
        console.error("[MIGRATION] FAILED:", sql.substring(0, 80), r.body);
      }
    }
    ...
    res.json({ results, tables: verify.body, policies: policies.body });
  });
