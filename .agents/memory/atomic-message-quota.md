...
For RPCs that return `messages.created_at`, preserve the live column's `TIMESTAMPTZ` type. `AT TIME ZONE 'UTC'` changes a timezone-aware value to `timestamp without time zone`, which makes PostgreSQL reject the function result and roll back message creation.